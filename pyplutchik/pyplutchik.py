...
def plutchik(scores,
    ...
    *ax*:
    ...
    if tags:
        tags = "Top tags : " + ", ".join(tags)

        ax.text(0.5, 1.02, tags, fontsize=11, fontstyle='italic', 
...
def plutchik_wheel(track_name, artist, emotion_vector, tags_list) :
    """Visualize the emotions of a given track"""
    idx_to_emotion = {               
        0: "anger",
        1: "anticipation",
        2: "disgust",
    ...
    emotions_to_plot = dict()
    for i, val in enumerate(emotion_vector):
        emotions_to_plot[idx_to_emotion[i]] = min(val,1.0)

    title_plot = f"{track_name} - {artist}"

    plutchik(emotions_to_plot, title=title_plot, tags=tags_list)